<template>
  <Dialog
      :visible="visible"
      :header="`Bulk Edit ${selectedItems.length} Favorites`"
      :modal="true"
      :closable="!saving"
  ...
  >
    <div class="dialog-content">
      ...
      <div class="summary-section">
        ...
        <span>You are editing <strong>{{selectedItems.length}}</strong> favorite location{{selectedItems.length !== 1 ? 's' : ''}}</span>
      </div>
...
const loadDistinctValues = async () => {
  try {
    const result = await favoritesStore.fetchDistinctValues()
...
const findSimilar = (value, list) => {
  // Simple similarity check - find items with Levenshtein distance <= 2
...
const proceedWithUpdate = async () => {
  ...
  try {
    const favoriteIds = props.selectedItems.map(item => item.id)

    const result = await favoritesStore.bulkUpdateFavorites(
        favoriteIds,
        updateCity.value,
    ...
    const successMsg = result.failedCount > 0
        ? `Updated ${result.successCount} of ${result.totalRequested} favorites (${result.failedCount} failed)`
        : `Successfully updated ${result.successCount} favorites`
    ...
    toast.add({
      ...
      detail: error.message || 'Failed to update favorites',
      life: 5000
    })
  } finally {
    saving.value = false
