    ...
    if [ -f "$file" ]; then
        case "$file" in
            ...
            *.md)
                ...
                # Match patterns like: ":1.8.0", "tag/1.8.0", "/1.8.0/"
                sed -i '' -E \
            ...
            .env.example)
                ...
                ;;
            *.properties)
                # For properties files: replace application.version property
                sed -i '' "s|^application\.version=$CURRENT_VERSION|application.version=$NEW_VERSION|g" "$file"
                ;;
            *.json)
                ...
                ;;
            *.conf)
                # For nginx.conf: replace in specific max-age patterns
                sed -i '' -E "s/(max-age=)$CURRENT_VERSION/\1$NEW_VERSION/g" "$file"
                ;;
            *)
