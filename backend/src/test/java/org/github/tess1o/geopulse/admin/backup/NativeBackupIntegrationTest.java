...
class NativeBackupIntegrationTest {
    ...
    @BeforeEach
    void createInstallations() throws Exception {
        source = installation("src");
        destination = installation("dst");
        ...
        try (Connection c = source.postgres().connect(source.postgres().database(), false)) {
            ...
            try (PreparedStatement s = c.prepareStatement("INSERT INTO oidc_providers(name,display_name,client_id,discovery_url,client_secret_encrypted,client_secret_key_id) VALUES ('test','Test','client','https://invalid.test',?,'v1')")) {
                s.setString(1, sourceKey.encrypt("OIDC secret"));
                s.executeUpdate();
            }
            try (PreparedStatement s = c.prepareStatement("INSERT INTO geocoding_provider_configs(id,name,display_name,type,url,headers_json,headers_key_id) VALUES (nextval('geocoding_provider_configs_seq'),'test','Test','nominatim','https://invalid.test',?,'v1')")) {
                s.setString(1, sourceKey.encrypt("{\"Authorization\":\"geocoding secret\"}"));
                s.executeUpdate();
            }
            try (PreparedStatement s = c.prepareStatement("INSERT INTO gps_source_config(id,user_id,source_type,active,token,payload_encryption_secret_encrypted,payload_encryption_secret_key_id) VALUES (?,?,'OWNTRACKS',true,'unchanged-api-token',?,'v1')")) {
                ...
                s.setObject(2, user);
                s.setString(3, sourceKey.encrypt("OwnTracks secret"));
                s.executeUpdate();
            }
    ...
    @Test
    void consistentSnapshotAndAllSecretsSurviveStagingAndAtomicActivation() throws Exception {
    ...
    @Test
    void wrongPasswordNeverCreatesStagingAndLiveSchemaDriftOnlyWarns() throws Exception {
        new NativeDatabaseBackup(source).write(archive, password, deadline(), "integration-backup");
        RestoreState state = new RestoreState();
        assertThatThrownBy(() -> new NativeDatabaseBackup(destination).prepare(archive, "wrong".toCharArray(), state, deadline(), p -> {
        })).isInstanceOf(Exception.class);
        assertThat(state.stagingDatabase).isNull();
        try (Connection c = destination.postgres().connect(destination.postgres().database(), false)) {
            execute(c, "ALTER TABLE users ADD COLUMN incompatible boolean");
        }
    ...
    @Test
    void brokenSecretsAbortPreparationWithoutClearingOriginalValues() throws Exception {
        try (Connection c = source.postgres().connect(source.postgres().database(), false)) {
            execute(c, "UPDATE oidc_providers SET client_secret_encrypted='invalid-ciphertext'");
        }
        new NativeDatabaseBackup(source).write(archive, password, deadline(), "integration-backup");
        RestoreState state = new RestoreState();
        state.fileName = archive.getFileName().toString();
        assertThatThrownBy(() -> new NativeDatabaseBackup(destination).prepare(archive, password, state, deadline(), p -> {
        })).isInstanceOf(Exception.class);
    ...
    @Test
    void cutoverTerminatesClientsAndPostgresRollsBackBothRenamesOnFailure() throws Exception {
        new NativeDatabaseBackup(source).write(archive, password, deadline(), "integration-backup");
        RestoreState state = prepare();
    ...
    private String decrypt(Connection c, String sql) throws Exception {
        return destinationKey.decrypt(scalar(c, sql), "v1");
    }
    ...
    private void setting(Connection c, String name, String value, boolean encrypted, KeyCipher key) throws Exception {
        try (PreparedStatement s = c.prepareStatement("INSERT INTO system_settings(key,value,value_type,category,encryption_key_id) VALUES (?,?,?,'backup',?)")) {
            s.setString(1, name);
            s.setString(2, encrypted ? key.encrypt(value) : value);
            s.setString(3, encrypted ? "ENCRYPTED" : "STRING");
            s.setString(4, encrypted ? "v1" : null);
            s.executeUpdate();
        }
    }
}
