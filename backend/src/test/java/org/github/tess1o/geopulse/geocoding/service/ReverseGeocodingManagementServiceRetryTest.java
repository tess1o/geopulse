...
class ReverseGeocodingManagementServiceRetryTest {
    ...
    @BeforeEach
    void setUp() {
        service = new ReverseGeocodingManagementService(
                geocodingRepository,
                providerFactory,
                configService,
                dtoMapper,
    ...
    void processReconciliationJob_retriesCircuitBreakerOpenAndSucceeds() {
        ...
        verify(reconciliationService, times(2)).reconcileWithProvider(userId, 1L, "Photon");
    ...
    void processReconciliationJob_marksFailureAfterMaxAttemptsAndContinues() {
        ...
        doReturn(successResponse).when(reconciliationService).reconcileWithProvider(userId, 1L, "Photon");
        doThrow(new RuntimeException("provider unavailable"))
                .when(reconciliationService).reconcileWithProvider(userId, 2L, "Photon");

        service.processReconciliationJob(jobId, userId, List.of(1L, 2L), "Photon");

        verify(reconciliationService, times(1)).reconcileWithProvider(userId, 1L, "Photon");
        verify(reconciliationService, times(3)).reconcileWithProvider(userId, 2L, "Photon");
    ...
    void processReconciliationJob_waitsBetweenItemsUsingInterItemDelay() {
        ...
        doReturn(firstResponse).when(reconciliationService).reconcileWithProvider(userId, 1L, "Photon");
        doReturn(secondResponse).when(reconciliationService).reconcileWithProvider(userId, 2L, "Photon");
        ...
        assertThat(elapsedMs).isGreaterThanOrEqualTo(90);
        verify(reconciliationService, times(1)).reconcileWithProvider(userId, 1L, "Photon");
        verify(reconciliationService, times(1)).reconcileWithProvider(userId, 2L, "Photon");
        verify(reconciliationProgressService).completeJob(jobId);
    }
}
