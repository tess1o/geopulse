...
public class GoogleTimelineImportStrategy implements ImportStrategy {
     ...
     * Since Google Timeline import only imports GPS points (not pre-computed timeline data),
    ...
    private void triggerTimelineGenerationForImportedGpsData(ImportJob job) {
        log.info("Triggering timeline generation for Google Timeline import for user {}", job.getUserId());
        ...
        try {
            ...
            dummyStay.setLocationName("Google Timeline Import Trigger");
            dummyStay.setStayDuration(0);
            dummyStay.setLocationSource(LocationSource.HISTORICAL);
            dummyStay.setTimelineVersion("google-timeline-import-trigger");
            ...
            log.info("Successfully queued timeline generation for Google Timeline import for user {}", job.getUserId());
            
        } catch (Exception e) {
            log.error("Failed to trigger timeline generation for Google Timeline import for user {}: {}", 
