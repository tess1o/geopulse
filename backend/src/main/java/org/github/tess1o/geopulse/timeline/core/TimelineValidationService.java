...
public class TimelineValidationService {
    ...
    public boolean passesAccuracyAndVelocityChecks(List<TrackPoint> cluster, 
        ...
        boolean velocityCheck = medianVelocity < config.getStaypointVelocityThreshold();
        
        if (!velocityCheck) {
            log.debug("Cluster failed velocity check: {} >= {}", medianVelocity, config.getStaypointVelocityThreshold());
        }
