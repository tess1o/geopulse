...
> If you are deploying on a Raspberry Pi, you can use the `tess1o/geopulse-backend:1.2.3-native-raspi` image.
> 
...
- ✅ Homelab IP access: `http://1.2.368.1.100:5555`
...
- **Homelab access (via IP address and/or local domain):**
  ...
  GEOPULSE_UI_URL=http://localhost:5555,http://1.2.368.1.100:5555,http://geopulse.local
  GEOPULSE_COOKIE_DOMAIN=           # Leave empty - nginx proxies everything
  GEOPULSE_AUTH_SECURE_COOKIES=false
  ```
