...
2. **JWT keys** (`10Mi`) generated by a backend initContainer
...
- JWT keys are generated by a backend initContainer at pod startup
...
Check backend initContainer logs:

```bash
kubectl get pods -l app.kubernetes.io/component=backend
kubectl logs -l app.kubernetes.io/component=backend -c generate-keys
```
